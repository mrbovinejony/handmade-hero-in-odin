...
main :: proc() {
	...
	for global_running {
		...
		game_update_and_render(&global_backbuffer, new_input)
...
game_update_and_render :: proc(game_offscreen_buffer: ^Game_Backbuffer, game_input: ^Game_Input){
...
/*win32_process_xinput_digital_button :: proc(p_new_button_state, p_old_button_state : ^Game_Button_State, p_button, p_button_bit: win.DWORD){
		p_new_button_state.half_transition_count = p_old_button_state.ended_down == p_new_button_state.ended_down ? 0 : 1
		p_new_button_state.ended_down = bool(p_button & p_button_bit)
		if p_new_button_state.ended_down{
			
		}
		fmt.println(p_new_button_state.half_transition_count)
}*/
