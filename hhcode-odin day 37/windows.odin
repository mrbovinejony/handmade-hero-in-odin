...
win32_process_pending_messages :: proc(
	p_keyboard_controller: ^Game_Controller_Input,
	p_state: ^Win32_State,
) {
...
win32_main_window_callback :: proc "stdcall" (
	...
	case win.WM_PAINT:
		...
		win32_update_window(device_context, dimension.width, dimension.height, global_backbuffer)
...
win32_get_input_file_location :: proc(
	p_state: ^Win32_State,
	slot_index: int,
	dest_count: int,
	dest: u8,
...
floor_real32_to_int32 :: proc(value: f32) -> i32 {
	result := i32(math.floor(value))
	return result
}

round_real32_to_int32 :: proc(p_number: f32) -> i32 {
	result := i32(math.round(p_number))
	return result
}

/*debug_load_bmp :: proc(p_filename : cstring16,  p_read_file : proc(p_filename : cstring16) -> Debug_Read_File_Result) -> Loaded_Bitmp{
	result : Loaded_Bitmp
	
	read_result := p_read_file(p_filename)
	fmt.println(read_result)

	if read_result.contents_size != 0{
		header := cast(^Bitmap_Header)read_result.contents
		pixels := cast(^u32)(cast(uintptr)read_result.contents + cast(uintptr)header.bitmap_offset)
		
		result.pixels = pixels
		result.width = header.width
		result.height = header.height

		source_dest := result.pixels
		if source_dest == nil{
		for y := i32(0); y < header.width; y += 1{
			for x := i32(0); x < header.height; x += 1{
